def visualize_ast(ast):
    ...
    def visit(node):
        ...
        if node_type == "Assign":
            ...
            edges.append({"from": assign_id, "to": var_id})

            value_id = visit(node.value)
            edges.append({"from": assign_id, "to": value_id})
            ...
            edges.append({"from": op_id, "to": left_id})
            edges.append({"from": op_id, "to": right_id})
            ...
            edges.append({"from": print_id, "to": expr_id})
            return print_id

        elif node_type == "IfElse":
            if_id = new_node("If-Else")

            cond_id = visit(node.condition)
            edges.append({"from": if_id, "to": cond_id})

            then_id = new_node("Then")
            edges.append({"from": if_id, "to": then_id})

            for stmt in node.if_body:
                stmt_id = visit(stmt)
                edges.append({"from": then_id, "to": stmt_id})

            else_id = new_node("Else")
            edges.append({"from": if_id, "to": else_id})

            for stmt in node.else_body:
                stmt_id = visit(stmt)
                edges.append({"from": else_id, "to": stmt_id})
            ...
            cond_id = visit(node.condition)
            edges.append({"from": while_id, "to": cond_id})

            body_id = new_node("Body")
            edges.append({"from": while_id, "to": body_id})

            for stmt in node.body:
                stmt_id = visit(stmt)
                edges.append({"from": body_id, "to": stmt_id})
